...
describe('GoalDetail Component with Progress Chart', () => {
  const mockGoal = {
    id: '123',
    userId: 'user123',
    title: 'Run 5K',
  ...
  test('renders goal details with progress chart', async () => {
    (useGoal as jest.Mock).mockReturnValue({
      data: mockGoal,
      isLoading: false,
      isError: false,
    ...
    // Check for date text - find the paragraph element under the Target Date heading
    const targetDateHeading = screen.getByText('Target Date');
    const targetDateValue = targetDateHeading.parentElement?.querySelector('p');
    expect(targetDateValue).toBeInTheDocument();
    expect(targetDateValue?.textContent).toContain('December');
    expect(targetDateValue?.textContent).toContain('2023');
    
    expect(screen.getByText('Train 3 times per week')).toBeInTheDocument();
    expect(screen.getByText('Created')).toBeInTheDocument();
    
    // Check for created date - find the paragraph element under the Created heading 
    const createdHeading = screen.getByText('Created');
    const createdValue = createdHeading.parentElement?.querySelector('p');
    expect(createdValue).toBeInTheDocument();
    
    // Check progress indicator - use a partial match for the percentage
    const progressElement = screen.getByText(/60/);
  ...
  test('renders achieved goal with different styling', () => {
    (useGoal as jest.Mock).mockReturnValue({
      data: mockAchievedGoal,
      isLoading: false,
      isError: false,
      error: null
    });
    ...
    // Check that achieved status is displayed
    ...
    // Verify chart is still present for achieved goals
    expect(screen.getByText('Progress Over Time')).toBeInTheDocument();
    expect(screen.getByTestId('recharts-responsive-container')).toBeInTheDocument();
  });
