...
if ($dockerfileContent -match "USER node") {
    Write-Output "✅ Running as non-root user"
} else {
    Write-Output "⚠️ Consider adding 'USER node' to run as non-root"
}

# Check if unnecessary file copying is avoided
if ($dockerfileContent -match "\.dockerignore") {
    Write-Output "✅ .dockerignore referenced"
} else {
